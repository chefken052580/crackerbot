...
botSocket.on('command', async (data) => {
  ...
  try {
    ...
    botSocket.emit('commandResponse', { success: true, response, target: 'bot_frontend' }); // Updated target
  } catch (error) {
  ...
  if (command.startsWith('Create Node.js API for')) {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "Generate a Node.js API with Express.js. Return only the code as a string, no explanations."
        },
        { role: "user", content: `Generate a Node.js API for ${name} with features: ${features || 'basic CRUD operations'}, including endpoints and basic logic.` }
      ],
      max_tokens: 1000,
    });
    const code = response.choices[0].message.content.trim();
    await redisClient.set(`api:${name}`, code); // Store in Redis
    return `Backend API for ${name} generated and stored:\n${code}`;
  }
