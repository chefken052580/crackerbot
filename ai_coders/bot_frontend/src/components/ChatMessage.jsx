// ai_coders/bot_frontend/src/components/ChatMessage.jsx (ESM, v2025-03-28-4)
...
const ChatMessage = ({ message, taskResult, onOptionClick, colorScheme, progress, setMessages, socket }) => {
  ...
  useEffect(() => {
    // Update progress from either message or prop, ensuring all updates are captured
  ...
  const renderProgressBar = () => {
    ...
    const gradient = isComplete
      ? 'linear-gradient(to right, #00ff9f, #ff00ff, #00ffff)' // Cosmic completion glow
      : `linear-gradient(to right, #ff00ff 0%, #00ffff 50%, #00ff9f ${clampedProgress}%, #333333 ${clampedProgress}% 100%)`;

    return (
      <div className="progress-bar-container mt-2 w-full max-w-md animate-supernova">
        <div
  ...
  const renderTaskResult = () => {
    ...
    return (
      <div className="task-result flex space-x-4 mt-4 animate-fade-in">
        {downloadLink && (
          <a
            href={downloadLink}
            download={fileName}
            className={`${colorScheme.accent} px-5 py-2 rounded-full hover:underline hover:text-[#00ff9f] transition-all duration-300 font-semibold tracking-wider border-2 border-[#ff00ff] shadow-[0_0_15px_#ff00ff] glow-effect`}
          >
            Download 🌠
          </a>
        )}
        {hasContent && (
          <button
            ...
            className={`${colorScheme.button} ${colorScheme.buttonText} px-5 py-2 rounded-full hover:scale-110 hover:shadow-[0_0_20px_#00ff9f] transition-all duration-300 border-2 border-[#00ff9f] shadow-[0_0_15px_#00ff9f] glow-effect`}
          >
            Preview 🚀
          </button>
        )}
  ...
  const renderOptions = () => {
    ...
    return (
      <div className="options-container flex flex-wrap gap-3 mt-4 animate-fade-in">
        {msg.options.map((option, idx) => {
          ...
          return (
            <button
              ...
              } rounded-full shadow-lg hover:scale-105 hover:shadow-[0_0_15px_#00ff9f] transition-all duration-300 border-2 border-[#00ff9f] glow-effect`}
            >
              {text}
            </button>
          );
  ...
  const renderProjects = () => {
    ...
    return (
      <div className="projects-container flex flex-col gap-4 mt-4 animate-slide-up">
        {msg.projects.map((project, idx) => (
          <div
            key={idx}
            className="project-item flex items-center justify-between bg-gray-900 p-3 rounded-lg shadow-md border border-[#ff00ff] glow-effect"
          >
            ...
            <div className="options-container flex gap-3">
              {project.options.map((opt, optIdx) => (
                <button
                  ...
                  className={`${colorScheme.bubble} px-4 py-2 text-base rounded-full shadow-md hover:scale-105 hover:shadow-[0_0_15px_#ff00ff] transition-all duration-300 border-2 border-[#ff00ff] glow-effect`}
                >
                  {opt}
                </button>
              ))}
  ...
  const messageClass = `${
    msg.type === 'user' || msg.type === 'command'
      ...
      ? `${colorScheme.bot} progress-message animate-supernova`
  ...
  return (
    <div
      ...
      data-user={msg.user}
    >
      <p className="break-words whitespace-pre-line text-base font-mono text-[#00ff9f]">{displayText}</p>
