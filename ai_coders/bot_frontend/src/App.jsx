...
import ErrorBoundary from './ErrorBoundary.jsx'; // Relative path to src/ErrorBoundary.jsx

const App = () => {
  const [activeTab, setActiveTab] = useState("chatroom");

  console.log(`[${new Date().toISOString()}] 🌌 Rendering App.jsx - Active Tab: ${activeTab}`);

  const renderContent = () => {
    console.log(`[${new Date().toISOString()}] 🌟 Rendering tab content: ${activeTab}`);
    switch (activeTab) {
      case "chatroom":
        return <ChatRoom />;
      case "wallet":
          ...
          <nav className="flex-1 flex flex-col mt-4 space-y-2">
            <button
              onClick={() => {
                ...
                console.log(`[${new Date().toISOString()}] 🔄 Switching to chatroom tab`);
              }}
            ...
            <button
              onClick={() => {
                setActiveTab("wallet");
                console.log(`[${new Date().toISOString()}] 🔄 Switching to wallet tab`);
              }}
              className={`flex items-center p-3 rounded-md ${
                activeTab === "wallet"
                  ? "bg-gray-700 text-neon-green shadow-md"
            ...
            <button
              onClick={() => {
                setActiveTab("token");
                console.log(`[${new Date().toISOString()}] 🔄 Switching to token tab`);
              }}
              className={`flex items-center p-3 rounded-md ${
                activeTab === "token"
                  ? "bg-gray-700 text-neon-green shadow-md"
